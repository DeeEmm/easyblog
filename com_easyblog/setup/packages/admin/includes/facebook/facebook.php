...
class EasyBlogFacebook extends EasyBlog
{
	...
	public function addOpenGraphTagsForPost(EasyBlogPost &$post)
	{
		...
		// Force always truncate the content for the meta truncation (getIntro) to prevent both sentences stick together issue
		$description = !empty($post->meta->description) ? $post->meta->description : $post->getIntro(false, true);
		$description = $this->formatDescription($description);
